...
def ensure_bot_running():
    """Start Telegram bot in background (works with gunicorn + local run)."""
    global _bot_thread_started
    if _bot_thread_started:
        return
    if not BOT_TOKEN or GROUP_CHAT_ID == 0:
        print("Warning: BOT_TOKEN or GROUP_CHAT_ID missing — bot not started")
        return
...
# Auto-start bot when this module is loaded (Railway / gunicorn)
ensure_bot_running()
